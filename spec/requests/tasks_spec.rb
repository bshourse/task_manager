...
describe 'Tasks API', type: :request do
  ...
  describe 'Get specific Task' do
    ...
    let!(:task) { FactoryBot.create(:task, task_name: 'Testname', project_id: project.id, user_id: user.id, performer_id: rand(1..3)) }
  ...
  describe 'Create new task' do
    let!(:user) { FactoryBot.create(:user, first_name: 'user_for_test', email: "test@jetrockets.com", password_digest: 123) }
    let!(:project) { FactoryBot.create(:project, project_name: 'project_for_test', user_id: user.id) }
    let!(:params) { {project_id: project.id, user_id: user.id, task_name: "Task001", description: "Тестовая задача для теста", performer_id: rand(1..3), due_date: "2020-02-23"} }
  ...
  describe 'Update task' do
    ...
    let!(:project) { FactoryBot.create(:project, project_name: 'project_for_test', user_id: user.id) }
    let!(:task) { FactoryBot.create(:task, task_name: 'Testname', project_id: project.id, user_id: user.id, performer_id: rand(1..3)) }
    let!(:params) { {task_name: "Тask001", description: "Тестовая задача для теста", performer_id: rand(1..3), due_date: "2020-02-23"} }

    it 'should update task' do
      patch "/api/v1/tasks/#{task.id}", params: params
      expect(task.reload.task_name).to eq(params[:task_name])
    end
